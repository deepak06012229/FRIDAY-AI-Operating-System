...
class FRIDAYBrain(QObject):
    ...
    def process_query(self, user_query):
        ...
        # 1. Check direct commands using IntentAnalyzer
        ...
        if local_intent:
            ...
            # Execute direct local command actions
            if intent_type == "exit_app":
                ...
                if facts:
                    ...
                    response_str = "No profile matrices logged yet, Commander."
                self.speak_requested.emit(response_str)
                self.memory.add_conversation_message("friday", response_str)
                self.response_completed.emit(user_query, response_str)
                return
                ...
                status_msg = f"Systems are nominal, Commander. CPU is at {cpu} percent. Memory is at {ram} percent."
        ...
        # 2. No local command match. Use Ollama first, Gemini fallback
        ...
        try:
            ...
            friday_response = ask_friday(user_query)
            ...
            try:
                ...
                friday_response = (
                    "Commander, I am unable to connect to either "
                    "the local AI model or the cloud AI service."
                )

        # 3. Parse embedded tool blocks
        try:
    ...
    def clean_json_from_response(self, text):
        """Removes the JSON block from text so TTS reads clean sentences."""
